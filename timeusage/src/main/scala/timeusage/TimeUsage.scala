...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(
      columnNames.map(name =>
        columnNames.indexOf(name) match {
          case 0 => StructField(name, StringType, false)
          case _ => StructField(name, DoubleType, false)
        }))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    columnNames.aggregate((List(), List(), List()))((lists, activity) => {
      val basic = List("t01", "t03", "t11", "t1801", "t1803")
      val work = List("t05", "t1805")
      val other = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
      if (basic.contains(activity.substring(0, 3)) || basic.contains(activity.substring(0, 5)))
        lists._1.+(activity)
      else if (work.contains(activity.substring(0, 2)) || work.contains(activity.substring(0, 5)))
        lists._2.+(activity)
      else if (other.contains(activity.substring(0, 2)))
        lists._3.+(activity)
      lists
    },
      (lists1, lists2) => (lists1._1 ++ lists2._1, lists1._2 ++ lists2._2, lists1._3 ++ lists2._3, )
    )
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = functions.when(df($"telfs") > 1 && df($"telfs") < 3, "working")
      .otherwise("not working").as("working")
    val sexProjection: Column = functions.when(df($"tsex") === 1, "male").otherwise("female").as("sex")
    val ageProjection: Column = functions.when(df($"teage") >= 15 && df($"teage") <= 22, "young")
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(id => (id.working, id.sex, id.age))
      .agg(
